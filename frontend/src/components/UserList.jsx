...
const UserList = () => {
  ...
  const isUserLive = (email) => liveUsers.some((u) => u.email === email);
  ...
  const getUserName = (user) => `${user.firstName || ""} ${user.lastName || ""}`.trim() || "Unknown User";
          ...
          {allUsers.map((user) => {
            ...
            return (
              <tr key={user.email} onClick={() => handleUserClick(user)} className="user-row">
                <td>{getUserName(user)}</td>
                <td>{user.email}</td>
                <td className={online ? "status-online" : "status-offline"}>{online ? "Online" : "Offline"}</td>
              </tr>
            );
          })}
        </tbody>
      ...
      {popupUser && (
        <div className="popup-overlay" onClick={closePopup}>
          <div className="popup-content" onClick={(e) => e.stopPropagation()}>
            <h3>User Info</h3>
            <p>
              <b>Name:</b> {popupUser.firstName} {popupUser.lastName}
            </p>
            <p>
              <b>Email:</b> {popupUser.email}
            </p>
            <p>
              <b>Mobile:</b> {popupUser.mobile}
            </p>
            <p>
              <b>Address:</b> {popupUser.address?.street}, {popupUser.address?.city}, {popupUser.address?.state},{" "}
              {popupUser.address?.country}
            </p>
            <p>
              <b>Login ID:</b> {popupUser.loginId}
            </p>
            <p>
              <b>Created At:</b> {popupUser.creationTime ? new Date(popupUser.creationTime).toLocaleString() : ""}
            </p>
